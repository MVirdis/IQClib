...
poly = poly_;
for iTF=2:nu+1
    psi(iTF,1) = tf(1,poly,Ts);
    poly = conv(poly, poly_);
end

% Inflate according to nq (kronecker product)
psi_ = tf(1,1,Ts)*zeros((nu+1)*nq,nq);
psi_(1:nq,1:nq) = tf(1,1,Ts)*eye(nq);
for iTF=1:nu
